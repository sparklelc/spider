            ...
            if(err){
                ...
                if (userLst){
                    userLst.map(function(item){
                        getFansRecur(item.uId);//递归爬其它人的信息
                    });
                }
                */
            }
...
function getUserLst(htmlContent,userId){
    var matched = htmlContent.match(/\"follow_list\s*\\\".*\/ul>/gm);   //   "follow_list    \"  XXXXXX/ul>""
    ...
    if(matched) {
        ...
        $("li[action-data]").map(function (index, item) {
            var userInfo = getUserInfo($,this);//获取每个用户的信息
            ...
            if(userInfo){
               if(!cachedUsers[userInfo.uId]){
                  ...
                  // if(userInfo.fansCnt > 100){
                       ...
                       userCnt++;
                       console.log(userCnt); //输出总共记录的user数目
                       saveUser(userInfo);
